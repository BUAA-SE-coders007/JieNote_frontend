<template>
  ...
  export default {
    ...
    methods: {
      async fetchPdf() {
        try {
          ...
          // 向后端发送请求获取 PDF 文件
          const response = await axios.get("https://jienote.top/article/readArticle", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
            params: {
              article_id: this.articleId, // 使用 props 传递的 articleId
            },
    ...
    mounted() {
      if (this.articleId) {
        this.fetchPdf(); // 调用方法获取 PDF 文件
      } else {
        console.error("未提供 articleId！");
      }
    },
  };
  </script>
